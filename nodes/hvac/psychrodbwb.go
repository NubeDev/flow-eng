...
func NewPsychroDBWB(body *node.Spec, _ ...any) (node.Node, error) {
	...
	n := &PsychroDBWB{body, "Metric/SI", false, 1}
...
func (inst *PsychroDBWB) Process() {
	...
	wetBulbT, _ := inst.ReadPinAsFloat(node.WetBulbTemp)
...
func (inst *PsychroDBWB) setSubtitle(units string, altitude float64) {
	subtitleText := ""
	if units == "Metric/SI" {
