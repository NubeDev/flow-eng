...
func NewPIDNode(body *node.Spec) (node.Node, error) {
	...
	enable := node.BuildInput(node.Enable, node.TypeBool, false, body.Inputs, false)
	processValue := node.BuildInput(node.ProcessValue, node.TypeFloat, nil, body.Inputs, false)
	setPoint := node.BuildInput(node.Setpoint, node.TypeFloat, 0, body.Inputs, true)
	minOut := node.BuildInput(node.MinOut, node.TypeFloat, 0, body.Inputs, true)
	maxOut := node.BuildInput(node.MaxOut, node.TypeFloat, 100, body.Inputs, true)
	inP := node.BuildInput(node.InP, node.TypeFloat, 1, body.Inputs, true)
	inI := node.BuildInput(node.InI, node.TypeFloat, 0.01, body.Inputs, true)
	inD := node.BuildInput(node.InD, node.TypeFloat, 0, body.Inputs, true)
	direction := node.BuildInput(node.PIDDirection, node.TypeBool, false, body.Inputs, true)
	interval := node.BuildInput(node.Interval, node.TypeFloat, 10, body.Inputs, true)
	bias := node.BuildInput(node.Bias, node.TypeFloat, 0, body.Inputs, true)
	manual := node.BuildInput(node.Manual, node.TypeFloat, 0, body.Inputs, true)
