...
func NewPACControl(body *node.Spec) (node.Node, error) {
	...
	setPoint := node.BuildInput(node.Setpoint, node.TypeFloat, 22, body.Inputs, true)
	clgOffset := node.BuildInput(node.ClgOffset, node.TypeFloat, 1, body.Inputs, true)
	htgOffset := node.BuildInput(node.HtgOffset, node.TypeFloat, 1, body.Inputs, true)
	stgUpDelay := node.BuildInput(node.StgUpDelay, node.TypeFloat, 10, body.Inputs, true)
	modeChangeDelay := node.BuildInput(node.ModeChangeDelay, node.TypeFloat, 15, body.Inputs, true)
	econoAllow := node.BuildInput(node.EconoAllow, node.TypeBool, false, body.Inputs, false)
	oaTemp := node.BuildInput(node.OATemp, node.TypeFloat, nil, body.Inputs, false)
	econoHigh := node.BuildInput(node.EconoHigh, node.TypeFloat, 20, body.Inputs, true)
	econoLow := node.BuildInput(node.EconoLow, node.TypeFloat, 10, body.Inputs, true)
	fanStatus := node.BuildInput(node.FanStatus, node.TypeBool, nil, body.Inputs, false)
	clgLockout := node.BuildInput(node.ClgLockout, node.TypeBool, false, body.Inputs, false)
	htgLockout := node.BuildInput(node.HtgLockout, node.TypeBool, false, body.Inputs, false)
