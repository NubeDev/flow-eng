...
func NewPACControl(body *node.Spec) (node.Node, error) {
	...
	clgOffset := node.BuildInput(node.ClgOffset, node.TypeFloat, nil, body.Inputs, nil)
	htgOffset := node.BuildInput(node.HtgOffset, node.TypeFloat, nil, body.Inputs, nil)
	stgUpDelay := node.BuildInput(node.StgUpDelay, node.TypeFloat, nil, body.Inputs, nil)
	modeChangeDelay := node.BuildInput(node.ModeChangeDelay, node.TypeFloat, nil, body.Inputs, nil)
	econoAllow := node.BuildInput(node.EconoAllow, node.TypeBool, nil, body.Inputs, nil)
	oaTemp := node.BuildInput(node.OATemp, node.TypeFloat, nil, body.Inputs, nil)
	econoHigh := node.BuildInput(node.EconoHigh, node.TypeFloat, nil, body.Inputs, nil)
	econoLow := node.BuildInput(node.EconoLow, node.TypeFloat, nil, body.Inputs, nil)
	fanStatus := node.BuildInput(node.FanStatus, node.TypeBool, nil, body.Inputs, nil)
	clgLockout := node.BuildInput(node.ClgLockout, node.TypeBool, nil, body.Inputs, nil)
	htgLockout := node.BuildInput(node.HtgLockout, node.TypeBool, nil, body.Inputs, nil)
