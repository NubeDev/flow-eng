...
func NewNumOutputSelect(body *node.Spec) (node.Node, error) {
	...
	inSelect := node.BuildInput(node.Selection, node.TypeFloat, nil, body.Inputs, false, false) // TODO: this input shouldn't have a manual override value
	input := node.BuildInput(node.In, node.TypeFloat, nil, body.Inputs, false, false)           // TODO: this input shouldn't have a manual override value
