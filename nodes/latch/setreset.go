...
func NewSetResetLatch(body *node.Spec) (node.Node, error) {
	...
	set := node.BuildInput(node.Set, node.TypeBool, nil, body.Inputs, false, true)     // TODO: this input shouldn't have a manual override value
	reset := node.BuildInput(node.Reset, node.TypeBool, nil, body.Inputs, false, true) // TODO: this input shouldn't have a manual override value
